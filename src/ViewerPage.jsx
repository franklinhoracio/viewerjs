...
const BASE = String(ORTHANC_BASE || "").replace(/\/+$/, "");
...
export default function ViewerPage() {
  ...
  const preloadingSeriesRef = useRef(new Set());

  const loadedCountBySeriesRef = useRef({});

  const [seriesProgressVisible, setSeriesProgressVisible] = useState(false);
  ...
  const realTotal = s.instances.length || 0;
  const loadedCount = loadedCountBySeriesRef.current[s.seriesId] || 1;

  return Math.min(realTotal, loadedCount);
};
  ...
  const showImage = async () => {
    ...
    try {
      ...
      if (requestId !== displayRequestIdRef.current) return;

      loadedImageIdsRef.current.add(imageId);

      loadedCountBySeriesRef.current[s.seriesId] = Math.max(
       loadedCountBySeriesRef.current[s.seriesId] || 0,
       iIdxRef.current + 1
      );
      ...
      if (counterRef.current) {
        counterRef.current.textContent = `${i}/${total}`;
      }
    ...
    setSeriesProgressVisible(true);

    clearTimeout(showImage.progressTimer);
    showImage.progressTimer = setTimeout(() => {
      setSeriesProgressVisible(false);
    }, 900);
  ...
  const next = (sIdxRef.current + delta + n) % n;
  goTo(next, 0);
};
...
const stepInstance = (delta) => {
  ...
  const next = (iIdxRef.current + delta + n) % n;
  goTo(sIdxRef.current, next);
};
  ...
  const startButtonScroll = (type, dir) => {
    ...
    if (type === "series") stepSeries(dir);
    else if (type === "instance") stepInstance(dir);
...
const preloadSeries = async (series) => {
  if (!series?.seriesId || !series.instances?.length) return;

  if (preloadingSeriesRef.current.has(series.seriesId)) return;
  preloadingSeriesRef.current.add(series.seriesId);
  ...
  try {
    ...
    const workers = Array.from({ length: concurrency }).map(async () => {
      while (index < series.instances.length) {
        const currentIndex = index++;
        const inst = series.instances[currentIndex];
        const imageId = wadouriFromInstanceId(inst.ID);

        if (loadedImageIdsRef.current.has(imageId)) continue;

        try {
          await cornerstone.loadAndCacheImage(imageId);
          loadedImageIdsRef.current.add(imageId);

          loadedCountBySeriesRef.current[series.seriesId] = Math.max(
            loadedCountBySeriesRef.current[series.seriesId] || 0,
            currentIndex + 1
          );
        } catch (err) {
          console.warn("No se pudo precargar instancia", imageId, err);
        }
      }
    });
  ...
  const loadStudy = async (studyId) => {
    ...
    console.log("LOAD STUDY ID:", studyId);
    console.log("ORTHANC BASE:", BASE);
    console.log("STUDY URL:", `${BASE}/studies/${studyId}`);
    ...
    try {
      ...
      for (const s of orderedSeries) {
        ...
        const modality = String(sTags.Modality || "").toUpperCase();
        // Filtrar series no renderizables
        if (["SR", "PR", "KO", "DOC"].includes(modality)) continue;
        ...
        const sop = String(t.SOPClassUID || "");

        const isSR = sop.startsWith("1.2.840.10008.5.1.4.1.1.88.");
        const isPR = sop === "1.2.840.10008.5.1.4.1.1.11.1";
        const isEncapsulatedPdf = sop === "1.2.840.10008.5.1.4.1.1.104.1";

        return !isSR && !isPR && !isEncapsulatedPdf;
      });
    ...
    const firstSeriesTotal = seriesWithInstances[0]?.instances?.length || 0;

    setInstanceProgress({
      current: 0,
      total: firstSeriesTotal,
      visible: firstSeriesTotal > 1,
    });
      loadedCountBySeriesRef.current = {};
      ...
      alert("Error cargando estudio: " + e.message);
    }
  };
...
if (wadouriMetaProvider) {
  ...
  const onVisibilityChange = () => {
    if (document.hidden) stopAllScrolling();
  };


    window.addEventListener("mouseup", stopAllScrolling);
    window.addEventListener("touchend", stopAllScrolling);
    window.addEventListener("touchcancel", stopAllScrolling);
    window.addEventListener("blur", stopAllScrolling);
    ...
    return () => {
      ...
      if (el) {
        ...
        window.removeEventListener("mouseup", stopAllScrolling);
        window.removeEventListener("touchend", stopAllScrolling);
        window.removeEventListener("touchcancel", stopAllScrolling);
        window.removeEventListener("blur", stopAllScrolling);
...
{seriesProgressVisible && instanceProgress.total > 1 && (
  <>

    <div className="vp-instance-energy">
  <div
    className="vp-instance-energy-fill"
    style={{
      width: `${
        instanceProgress.total > 0
          ? (instanceProgress.current / instanceProgress.total) * 100
          : 0
      }%`,
    }}
  />
        ...
        {/* Contador series */}
        <div ref={seriesCounterRef} className="vp-series-counter" aria-label="series-counter">
          0/0
        </div>
      </div>
...
<ViewerFooter publicConfig={publicConfig} />
